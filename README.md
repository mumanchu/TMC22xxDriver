...
There are already _many_ TMC2209 libraries out there. So just to confuse you even more, here is another one.
...
> **DREAM ON** \
> One day, a new generation of chips might handle the acceleration/deceleration curves internally, so you just select the curve (Trapeziodal or S-Curve) and acceleration, and write the number of steps into a register. The chip then manages the movement itself, signalling when the movement is complete. Then you won't need the `MiniStepper` library :-)
...
- [Stall Detection, Does it really work?](#stall-detection)
- [One Time Programming (OTP)](#otp)
- [TMC22xx Data Sheets](#data-sheets)

_\<i think you've written too much, nobody's ever going to read it - ed\>_
...
Up to 4 motors can be connected to the UARTs, sharing the same TX and RX lines. All chips listen to the messages sent by the MCU, but only the addressed chip will respond. On 3D printer boards, each TMC2208 chip has its address hard-wired on the board. The AD0/AD1 pins can also be used as MS0/MS1 to select the microsteps, but because UART programming is used the microsteps are progammed via the `CHOPCONF` register's `MRES` bits.

Because there is only one pin for both TX and RX, it's necessary to use an external 1K ohm resistor to prevent conflicts between the MCU's TX driver and the TMC chip UART output. Some of the stand-alone driver boards have this resistor already fitted.

![TMC22xx TX Resistor](https://github.com/mumanchu/mumanchu/blob/main/assets/tmc2209-tx-rx.jpg)


A few of the 3D printer main boards do not have the RX connection, so the TMC22xx registers cannot be read - they are all 'write-only'.
...
It is easy to address more than 4 TMC chips. The manual (p21) recommends an "analog switch" 74HC4066 to select individual chips with individual "chip select" outputs. That allows MC0/MC1 to be used for microstep selection, but it uses one additional output per chip.

A better way would be to use a single output to control a 1-pole 2-way (single pole double throw SPDT) analog switch to select a bank of four TMC chips, e.g. bank0 = 0..3 and bank1 = 4..7, so node addresses 0..3 can be used for both banks of four chips. This could be done with a cheap DG41xx analog multiplexer chip (Vishay or Maxim), or even an old CD4016, CD4051, CD4053 etc.
...
(3) Use 5V for a 5V MCU, or 3.3V for a 3.3V MCU, else "Bang!". See Disclaimer. 3.3V usually worka for 5V MCUs, but not the other way round!

For validating the motor movement, a disc encoder with 20 slots was fitted to the motor's spindle. This used an "opto interrupter" (like the ITR9606 or ITR9608) to provide 20 pulses-per-revolution into the TACHO (tachometer) input.
...
The motor power VM is supplied by an external 12V or 24V power supply (min. 2A for 1 motor). VM also supplies the internal logic via the TMC chip's internal 5V regulator. UART communications will not work without it.

![Prototype Shield Photo](https://github.com/mumanchu/mumanchu/blob/main/assets/prototype-shield.jpg)


> [!CAUTION]
> My first shield prototype, with a 24V motor supply, had a hidden short between a motor control output pin and an MCU input. 24V on the MCU input immediately destroyed it. 12V or 24V and 3.3V MCU I/Os do not mix!
...
This library does not control the motor with the STEP/DIR pins, but it _can_ use the TMC's 'internal step pulse generator' to run the motor forwards or backwards at a particular velocity. This makes it run like a speed-controlled motor. 
...
The TMC chips have the traditional STEP, DIR and EN stepper motor control signals. STEP/DIR control is not handled by this library.
...
Another disadvantage of microstepping is that if power is cut to the drivers, the motor wil move to the next (or nearest) fullstep position because motors always lock at the fullstep position. This makes re-starting an operation at the _exact_ place where is stopped is very difficult. The TRINAMIC controllers have an `MSCNT` register which holds the microstep position, so clever software is needed to restore the exact microstep position when power is returned to the drivers. This is described in the TMC2209 data sheet p15, 'Restart the Stepper Motor Without Position Loss'.

<!-- TODO add feature to do this? -->


### Configuring microsteps, p33

When UART communications is used, the TMC controller chips configure the microstep setting via the `MRES` bits (Microstep Resolution, p33) in the `CHOPCONF` register. The library has a `bool setMicrosteps(uint microsteps)` method to do this. But first you must enable `MRES` with the `mstep_reg_select` bit in the `GCONF` register, see `setConfiguration()`. If this bit not set, it will attempt to use the MS0 and MS1 pins for the microstep setting.
...
It's a good idea to check for errors regularly at runtime, so the movement can be stopped if something nasty happens. Using the DIAG output to generate an interrupt is the fastest way to react to an error.

<!-- ========================================================================================== -->


<a name="stall-detection"></a>
## Stall Detection, Does it really work?

The TMC chips measure the current through the motor's coils. This can be used to detect if the motor has "stalled". For example, if it has hit the end-stop, if your tie is caught in the pulley wheel and you are being strangled, or if your finger is being shredded by drive belt.
...
> [!CAUTION]
> The factory-set clock frequency tuning `FCLKTRIM` cannot be changed, `programOTP()` does not program these bits.
...
You may find this blog entry here, when it's finished... \
